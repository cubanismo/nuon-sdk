//
// raw2buf: convert a .raw triangle file into an 
// MML execute buffer
//
...
Usage(void)
{
	fprintf(stderr, "Usage: raw2buf [-flip][-scale size][-rx angle][-ry angle][-rz angle][+x dist][+y dist][+z dist] inputfile outputfile\n");
	exit(2);
}
...
ReadRawFile(char *filename)
{
	...
	for (;;) {
		...
		if (flip_flag)
			r = fscanf(f, "%lf %lf %lf %lf %lf %lf %lf %lf %lf\n",
		...
		if (r != 9) {		// not a proper triangle description
...
WriteIFile(char *filename, int numtris)
{
	...
	for (i = 0; i < numtris; i++) {
		...
		fprintf(f, "\t.dc.s\t3,_default_material,0,%d\n", M3D_POLY_ENTRY);
