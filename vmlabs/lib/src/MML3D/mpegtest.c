...
#define _OLD_JOYSTICK  // use old joystick structures
//#define NO_VIDEO
//#define NO_MPEG
 ...
 * 
 */
/* $Id: mpegtest.c,v 1.6 2001/10/18 22:28:17 ersmith Exp $ */
...
#ifdef NO_MPEG
asm(".segment sdram\n.export _catpix128_start\n_catpix128_start: .binclude \"cat2.bin\"\n.text\n");
#else
asm(".segment sdram\n.export _catpix128_start\n_catpix128_start: .binclude \"catmpeg.bin\"\n.text\n");
#endif
...
{
    mmlDisplayPixmap pmap;

#ifdef NO_MPEG
    pmap.dmaFlags = ((128/8)<<16) | DMA_PIXEL_READ | (2<<4);
#else
    pmap.dmaFlags = ((128/16)<<16) | DMA_MPEG_READ | (1<<11) | (1<<9);
#endif
    pmap.memP = catpix128_start;
    pmap.wide = 128;
    pmap.high = 128;

    m3dInitMaterialFromColor(&red, mmlColorFromRGB(200, 0, 0));
    m3dInitMaterialFromColor(&blue, mmlColorFromRGB(0, 0, 200));
    m3dInitMaterialFromPixmap(&cat_mipmap[0], &pmap);
