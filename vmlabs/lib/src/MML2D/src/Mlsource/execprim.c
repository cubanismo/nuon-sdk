...
extern mrpStatus LoadCOFF( );
extern mrpStatus ScrollUp( );
extern mrpStatus Copy32to16( );
extern mrpStatus CopyRectFast( );
extern mrpStatus CopyRect16( );
extern mrpStatus CopyRGBFast( );
extern mrpStatus DrawPointDispatch( );
extern mrpStatus SmallFillDispatch( );
extern mrpStatus MovePixDispatch( );
extern mrpStatus DrawLinePlus();
extern mrpStatus DrawEllipsePlus();
extern mrpStatus TexBlt();
extern mrpStatus TexBlend();
extern mrpStatus TexAlpha();
...
int	mrpProc[ ] [2]= {
	...
	{(int)FillMpeg, eFillMpeg },
	{(int)FillClut, eFillClut },
	{(int)CopyRectFast, eCopyRectFast }, 
	{(int)CopyRGBFast, eCopyRGBFast }, 
	{(int)DrawPointDispatch, eDrawPoint },  
	{(int)SmallFillDispatch, eSmallFill },
	{(int)MovePixDispatch, eMovePix },
	{(int)DrawLinePlus, eDrawLinePlus },
	{(int)DrawEllipsePlus, eDrawEllipsePlus },
	{(int)TexBlt, eTxBlt }, 
	{(int)TexBlend, eTxBlend },
	{(int)TexAlpha, eTxAlpha },
	{(int)ScrollUp, eScrollUp }, 
...
{
	if( gcP->sequence == NULL || prim == eExecSequence )
#if( USE_DISPATCHER == 1 )
{
...
*/		
	...
	{
		mmlStatus stat = SeqAddCmd( gcP->sequence, prim, (uint32)paramBlockP,
			option2, option3 );
		assert( stat == eOK );
		return stat;
	}
