...
11/24/98 rwb modify to handle transparent source pixels in 8bitClut and 1555RGB pixels.
In dtram, an alpha value of 0xFF means transparent.
*/
void ColorCvrtTrans( indexBlock* inP, indexBlock* outP, int nPix, int endIn, int endOut, int srcType )
{	
	Push( v6 )
	Push( v5 )
	...
	s3:	GetDRamMM( v6, xy, rx )	//V = rBlock--;
		asm( "btst #29, r24 \n jmp ne, (%0)"::"r" (s4P));
		SetRegister( r23, 0xFF000000 )		// transparent pixel
		...
		SetRegister( r23, 0 )
	s4:
		PutDRamAlphaMM( v5, uv, ru )	//wBlock-- = Z;
		DecCtr( rc0 )
		Repeat( c0, s3P )
		//while( --nPix > 0 );
		Pop( v2 )
