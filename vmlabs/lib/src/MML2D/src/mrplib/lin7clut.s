...
//	_draw_line7clut(environs, paramBlockP, 0, 0);
//  Used for pixel data type 3 (8-bits) which uses a color lookup table. This line type
//      interpolates 2 colors between the 2 line endpoints. The 2 colors are the boundaries of
//      a color gradation in the palette. So the algorithm for this line type assigns a color to
//      a line span depending on the span's position along the line length, starting with color0 at the
//      first boundary and ending with color1 at the second boundary.
...
//pline_ptr:	.dc.s   0
//cs:			.dc.s   0,0,0
//cinterp:		.dc.s   0,0,0,0			// delta_x, delta_y, ?, ?
...
idxToLenOf		= genbufAddrOf + (1 * szLong)
GidxToLen		= _FALSE	// global storage flag
...
; register aliases
	...
	dmastuff = v3
;	dma_count = r14
	dma_len = r14   // ALWAYS set this to an even value. This is the X length of the DMA transfer 
...
; clip thangs
    ...
    flag_extra_dma = r31  // for every scan line,
                          // bit 0 is set if there is an extra leading pixel at the dma start
                          // bit 1 is set for the last dma, if bit 31 is set which means there is
                          //       an extra trailing pixel
    
    clutToLen = r30     // abs(color0-color1) / [(x1-x2)^2 + (y1-y2)^2], Note: The endpoints are extended
                        // in a half-circle with radius=0.5*Total_Line_Width. This is excluded from the
                        // computation.
...
;-----------------------------------------------------------------
	...
	.if(GidxToLen == _FALSE)
	setParamAddr	idxToLenOf, idxToLen
	.endif
...
{
	ld_io	(r0),r17		;pick up scales
	add	#8,r0
}
	ld_io	(r0),r0			;pick up rotate angle
...
//	mv_s	#pline,mline		;Set mline pointer
	...
	ld_s	(mline),mline
	nop
    st_s    #NumPts,rc0         ;use counter to trigger loading of longer pline list
...
//	mv_s	#object+20,r0
	...
	ld_io	(r0),width				;get back thickness and width that were mashed
	nop
	lsr	#2,width,r31
	bits	#7,>>#0,width
...
//	mv_s    #object+8,r0
    ...
    ld_io   (mline),r4      ;fetch the colour
    nop
...
;	ld_s    object+20,r0    ;get existing width
	...
	nop
	add		#20,r30
	ld_io	(r30),r0		// r30 contains #object+20
		
    ld_io    (mline),r4      ;get new width  
...
{
	ld_io	(r0),r16		;pick up positional X and Y
	add	#16,r0
}
	nop
//	mv_s	#trig,r1
...
{
    ld_io    (r0),r4         ;get colour 1
    add #4,r0
}
{
    ld_io    (r0),r12         ;get colour 2       
...
no_colour_swap:
    ...
    lsr     #8,r4           // shift out blending value in first byte of color
    lsr     #8,r12          // shift out blending value in first byte of color

	.if(B==0)
	STORE	Giv0, st_s, iv0, r12, r30
	.else
...
}
{
	...
	ls	r1,r0,sineline						;r0 contains the llength of the lline, 4.28
	mv_s	sineline,cosline		;the cosine will be useful too
// ===================================================
//  compute idxToLen value before entering aal_outer:
//  abs(color0-color1)/sqrt[(delta_y)^2 + (delta_x)^2]

    push v3
    // numerator:
	LOAD Giv0, ld_s, iv0, r12, r12     // get color0
	LOAD Giv1, ld_s, iv1, r13, r13     // get color1
    sub r12,r13,r15
    abs r15
    lsl  #24,r15                        // in 8.24 format

    // divide:
    mul sineline,r15,>>#24,r15               // in 4.28 format
    nop

    // store:
    STORE	GidxToLen, st_s, idxToLen, r15, r12
...
// moved this to neswap label so as to check dmaFlags transfer type only once
	...
	st_s	#0,rx
{
    ...
    // load idxToLen into clutToLen
    LOAD    GidxToLen, ld_s, idxToLen, clutToLen, clutToLen
...
// For DMA data transfer type 3, X position & X length must be divisible by 2. So ....
// Test dma_xpos (dma X position) & r24 (total length of x-span) for being even
// and adjust them if they're not. Since dma_len is always even, the tests can be done once at the
// start of processing a scan line

    sub flag_extra_dma,flag_extra_dma

    btst #16,dma_xpos
    bra eq,test_x_len,nop   //dma_xpos is even, so test x length

    eor #$10000,dma_xpos    //dma_xpos is odd, so set it to even
    bset #0,flag_extra_dma  // and set indicator bit for a dummy leading pixel at start of dma
    add  #1,r24             // add 1 to total width of span

test_x_len:

    btst #0,r24
    bra eq,aal_inner,nop    //"dma x length" is even

    add  #1,r24             //"dma x length" is odd, so set it to even
    bset #31,flag_extra_dma // and set indicator bit for the end of dma
...
//	bra	ge,notrunc					// HACK NOTE1 >=0, no need to truncate
	bra	gt,notrunc					;>0, no need to truncate
}
	st_s	r0,acshift
	nop

    // this is the last dma for the scan line
	add	r24,dma_len			;adjust length
    btst #31,flag_extra_dma     // Is the indicator for a dummy trailing pixel set?
    bra eq,notrunc,nop          // No
    bset #1,flag_extra_dma      // Yes, then set indicator bit for a trailing extra pixel
//	add	#1,dma_len          // TAJ-removed this since this is really a hack for the case where r24==dma_len
                            // Jeff probably wanted to save clock ticks.
                            // See HACK NOTE1 below
...
// Check if a dma read is necessary, because of extra pixels. These pixels will not be changed and therefore
// should be read in and writtten out unchanged.
// Note to above comment: Nope. Need to read all those pixels because of beveling at endpoints. The next dma
// write should write back the original values of the pixel-positions which are beveled-off.
    
/*  and #3,flag_extra_dma,r1    // test if bit 0 or 1 of flag_extra_dma is set
    bra eq,aal_inner_inner,nop  // If not, then a dma-read is not necessary. The pixel value will just
                                //    be taken from the clut(the color-lookup table).
*/	
...
/*{
 	ld_s	dmaFlags,r1				;Dest DMA type                                                                                                                                     
	jsr		dma_go					;wait for DMA available
}*/
	...
	ld_s	mdmactl,r0				;get dma status on the way
	st_s	#0,ru
{
...
; loop for this scanline
    ...
    cmp     #0,r24          // HACK NOTE1: and then added this compare to compensate
    jmp eq,aal_done,nop     
