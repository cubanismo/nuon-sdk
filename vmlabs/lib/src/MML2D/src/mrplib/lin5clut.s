...
//	_draw_line5clut(environs, paramBlockP, 0, 0);
//  Variant of _draw_line5 used for pixel data type 3 (8-bits) which uses a color lookup table
//	r0 = environment data
...
//iv0:			.dc.s   0,0,0,0  // NOTE: For clut mode, only the first byte is filled with the color index
//iv1:			.dc.s   0,0,0,0  // NOTE: For clut mode, only the first byte is filled with the color index
...
; clip thangs
    ...
    clutToWidth = r30     // (Total Number of Clut Entries per Color) / (0.5 * Total Line Width)
...
; _draw_line5clut: draw fat neon antialiased line
...
{
    ld_s    (r0),r4         ;get colour 1
    add #4,r0
}
{
    ld_s    (r0),r12         ;get colour 2       
...
// Test dma_xpos (dma X position) & r24 (dma X length) for being even
// and adjust them if they're not
...
//  Load clutToWidth value before entering aal_outer
