...
; MGL polygon rasterization routines - paletted textures with chromakey
...
.export _RasterSTPK_size
	_RasterSTPK_size = _RasterSTPK_end - _RasterSTPK
.export _RasterSTPK
.import _FloorDivMod
...
`positiveendingw:
	...
	{
	mv_s	v6[0], v0[1]							; V0[1] contains starting z
...
; v0[0]	= unused
; v0[1]	= z (LOOP INPUT)
; v0[2]	= saved linpixctl
; v0[3]	= DMA pointer (LOOP INPUT/OUTPUT)
;
; v1[0]	= unused
; v1[1]	= unused
; v1[2]	= unused
; v1[3]	= dz/dx (LOOP INPUT)
;
; v2[0]	= unused
; v2[1]	= unused
; v2[2]	= unused
; v2[3]	= du/dx (LOOP INPUT)
;
; v3[0]	= unused
; v3[1]	= unused
; v3[2]	= unused
; v3[3]	= dv/dx (LOOP INPUT)
;
; v4[0]	= scratch for chroma reject
; v4[1]	= scratch for chroma reject
; v4[2]	= unused
; v4[3]	= unused
;
; v5[0]	= scratch for chroma reject
; v5[1]	= scratch for chroma reject
; v5[2]	= scratch for chroma reject
; v5[3]	= unused
;
; v6[0]	= index; Y
; v6[1]	= Cr
...
; v6[3]	= alpha; z
;
; v7[0]	= scratch for chroma reject
; v7[1]	= scratch for chroma reject
; v7[2]	= scratch for chroma reject
; v7[3] = scratch for chroma reject

`RasterPreLoop:
	ld_s	(linpixctl), v0[2]						; save linpixctl
	ld_p	(uv), v6								; load index
`RasterLoop:
	st_s	#$10400000, (linpixctl)					; set linpixctl for clut access
	ld_pz	(v6[0]), v6								; load color with alpha
	st_s	v0[2], (linpixctl)						; set linpixctl for dma buffer access
	cmp		#$ff000000, v6[3]						; check alpha
	{
	bra		eq, `ChromaAccept						; reject transparent alpha values
	mv_s	v0[1], v6[3]							; copy z to output pixel
	dec		rc0										; decrement number of pixels remaining in subspan
	}
	st_pz	v6, (v0[3])								; store pixel with z; ignored later if rejected now
	nop

`ChromaDMAwait:
	ld_s	(mdmactl), v4[0]						; read dma control register
	ld_s	(_MPEPolygonDMASourcePointer), v5[0]
	{
	and		#$f, v4[0]								; check for dma activity
	ld_s	(_MPEPolygonY), v7[3]
	}
	{
	bra		ne, `ChromaDMAwait
	sub		v5[0], v0[3], v5[1]						; number of bytes to dma
	ld_s	(_MPEPolygonX), v7[2]
	}
	{
	asr		#2, v5[1]								; number of pixels to dma
	ld_s	(_MPESDRAMPointer), v7[1]
	}
	{
	addm	v7[2], v5[1], v4[1]
	lsl		#16, v5[1], v5[2]
	ld_s	(_MPEDMAFlags), v7[0]
	}
	{
	st_s	v5[0], (_MPEMDMACmdBuf+16)
	or		v5[2], v7[2]							; set x size
	}
	{
	bset	#16, v7[3]								; set y size to 1
	ld_s	(_MPEPolygonDMASize), v4[0]
	}
	{
	st_v	v7, (_MPEMDMACmdBuf)
	eor		#DMA_CACHE_EOR, v5[0]					; flip dma buffers
	}
	{
	st_s	#_MPEMDMACmdBuf, (mdmacptr)					; initiate dma
	add		#1, v4[1]
	}
	{
	st_s	v5[0], (_MPEPolygonDMASourcePointer)	; store dma pointer
	sub		v5[1], v4[0]
	}
	{
	mv_s	v5[0], v0[3]							; store dma pointer
	sub		#1, v4[0]
	}

`ChromaRejectLoop:
	{
	bra		c0eq, `RasterPostLoop					; exit RasterLoop if no pixels remaining in subspan
	st_s	v4[0], (_MPEPolygonDMASize)				; store max dma count
	add		v1[3], v0[1]							; increment z
	addr	v2[3], ru								; increment ru
	}
	{
	st_s	v4[0], (rc1)							; store max dma count
	sub		#1, v4[0]								; decrement max dma count
	addr	v3[3], rv								; increment rv
	}
	{
	st_s	v4[1], (_MPEPolygonX)					; store x
	add		#1, v4[1]								; increment x
	}
	ld_p	(uv), v6								; load index
	st_s	#$10400000, (linpixctl)					; set linpixctl for clut access
	ld_pz	(v6[0]), v6								; load color with alpha
	...
	cmp		#$ff000000, v6[3]						; check alpha
	{
	bra		ne, `ChromaRejectLoop					; rejecting transparent alpha values
	mv_s	v0[1], v6[3]							; copy z to output pixel
	dec		rc0										; decrement number of pixels remaining in subspan
	}
	st_pz	v6, (v0[3])								; store pixel with z; ignored later if rejected now
...
`ChromaAccept:
	{
	bra		c0ne, `RasterLoop						; loop back for next pixel if pixels remaining in subspan
	addm	v1[3], v0[1]							; increment z
	add		#4, v0[3]								; increment dma pointer
	addr	v2[3], ru								; increment ru
	dec		rc1										; decrement number of pixels remaining in dma buffer
	}
	addr	v3[3], rv								; increment rv
	ld_p	(uv), v6								; load index
...
.export _RasterSTFPK_size
	_RasterSTFPK_size = _RasterSTFPK_end - _RasterSTFPK
.export _RasterSTFPK
.import _FloorDivMod
.import _MPEMDMACmdBuf
.import _MPEPolygonGradient
...
;
; Features: clut, bilerp, chromakey
;
; ru	= texture u-coordinate (LOOP INPUT)
; rv	= texture v-coordinate (LOOP INPUT)
;
...
; v3[3]	= dv/dx (LOOP INPUT)
;
; v4[0]	= intermediate Y; scratch for chroma reject
; v4[1]	= intermediate Cr; scratch for chroma reject
; v4[2]	= intermediate Cb
; v4[3]	= unused
;
; v5[0]	= intermediate Y; scratch for chroma reject
; v5[1]	= intermediate Cr; scratch for chroma reject
; v5[2]	= intermediate Cb; scratch for chroma reject
; v5[3]	= unused
;
; v6[0]	= final Y
; v6[1]	= final Cr
...
; v6[3]	= z (LOOP INPUT)
;
; v7[0]	= UL index; UL Y; scratch for chroma reject
; v7[1]	= UL Cr; scratch for chroma reject
; v7[2]	= UL Cb; scratch for chroma reject
; v7[3] = UL alpha; scratch for chroma reject

`RasterPreLoop:
	ld_s	(linpixctl), v0[2]						; save linpixctl

`RasterLoop1:
	addr	#1<<16, rx								; point xy at UR
	{
	ld_p	(xy), v1								; load UR index
	addr	#1<<16, ry								; point xy at LR
	}
	{
	ld_p	(xy), v3								; load LR index
	addr	#-1<<16, rx								; point xy at LL
	}
	{
	ld_p	(xy), v2								; load LL index
	addr	#-1<<16, ry								; point xy at UL
	}
	{
	ld_p	(xy), v7								; load UL index
	addr	#1<<16, rx								; point xy at UR
	}
	{
	st_s	#$10400000, (linpixctl)					; set linpixctl for clut access
	addr	v2[3], rx								; increment rx
	}
	{
	ld_pz	(v7[0]), v7								; load UL color with alpha
	addr	v3[3], ry								; increment ry
	}
	ld_p	(v1[0]), v1								; load UR color
	ld_p	(v2[0]), v2								; load LL color
	ld_p	(v3[0]), v3								; load LR color

`RasterLoop2:
	cmp		#$ff000000, v7[3]						; check UL alpha
	bra		eq, `ChromaAccept						; rejecting transparent alpha values
	{
	st_s	#$10400000, (linpixctl)					; set linpixctl for clut access
	sub_p	v7, v1, v4								; work on bilerp
	}
	{
	sub_p	v2, v3, v5								; work on bilerp
	mul_p	ru, v4, >>#30, v4						; work on bilerp
	ld_p	(xy), v1								; load next UR index
	addr	#1<<16, ry								; point xy at next LR
	}
...
`ChromaRejectLoop:
	...
	{
	bra		c0eq, `RasterPostLoop					; exit RasterLoop1 if no pixels remaining in subspan
	st_s	v4[0], (rc1)							; store max dma count
	sub		#1, v4[0]								; decrement max dma count
	addr	v3[3], rv								; increment rv
	}
	ld_p	(uv), v7								; load index
	{
	st_s	v4[1], (_MPEPolygonX)					; store x
	add		#1, v4[1]								; increment x
	}
	ld_pz	(v7[0]), v7								; load color with alpha
	nop												; to get rid of this, ChromaRejectLoop needs to be restructured
													; ... this would require more iram, and iram dedicated to
													; ... rasterizer is nearly all used
	cmp		#$ff000000, v7[3]						; check alpha
	{
	bra		ne, `ChromaRejectLoop					; reject transparent alpha values
	ld_s	(ru), v5[0]
	}
	{
	bra		`RasterLoop1							; back to outer loop; accept minor redundancy, as iram
													; ... dedicated to rasterizer is nearly all used
	ld_s	(rv), v5[1]
	}
	st_s	v5[0], (rx)
	st_s	v5[1], (ry)
...
`ChromaAccept:
	{
	mul_p	ru, v5, >>#30, v5						; work on bilerp
	ld_p	(xy), v3								; load next LR index
	addr	#-1<<16, rx								; point xy at next LL
	}
	ld_p	(v1[0]), v1								; load next UR color
	{
	add_p	v2, v5									; work on bilerp
	ld_p	(v3[0]), v3								; load next LR color
	}
	{
	add_p	v7, v4									; work on bilerp
	ld_p	(xy), v2								; load next LL index
	addr	#-1<<16, ry								; point next xy at UL
	}
	{
	sub_p	v4, v5									; work on bilerp
	ld_p	(xy), v7								; load next UL index
	addr	#1<<16, rx								; point xy at next UR
	}
	{
	mul_p	rv, v5, >>#30, v5						; work on bilerp
	ld_p	(v2[0]), v2								; load next LL color
	}
	{
	addr	v2[3], ru								; increment ru
	dec		rc0										; decrement number of pixels remaining in subspan
	dec		rc1										; decrement number of pixels remaining in dma buffer
	ld_pz	(v7[0]), v7								; load next UL color with alpha
	}
	{
	bra		c0ne, `RasterLoop2						; loop back for next pixel if pixels remaining in subspan
	addr	v3[3], rv								; increment rv
	}
	{
	add_p	v4, v5, v6								; bilerp complete
	st_s	v0[2], (linpixctl)						; set linpixctl for dma buffer access
	addr	v2[3], rx								; increment rx
	}
	{
	st_pz	v6, (v0[3])								; store pixel with z
	addm	v1[3], v6[3]							; increment z
	add		#4, v0[3]								; increment dma pointer
	addr	v3[3], ry								; increment ry
	}
...
.export _RasterSTPKI_size
	_RasterSTPKI_size = _RasterSTPKI_end - _RasterSTPKI
.export _RasterSTPKI
.import _FloorDivMod
.import _MPEMDMACmdBuf
.import _MPEPolygonGradient
...
;
; Features: clut, intensity lighting, chromakey
;
; ru	= texture u-coordinate (LOOP INPUT)
; rv	= texture v-coordinate (LOOP INPUT)
;
...
`ChromaAccept:
	{
	bra		c0ne, `RasterLoop						; loop back for next pixel if pixels remaining in subspan
	mul_p	v4[3], v6, >>#30, v6					; modulate filtered color
	add		v5[3], v4[3]							; increment I
	mv_s	v0[1], v6[3]							; copy z to output pixel
	addr	v2[3], ru								; increment ru
	dec		rc1										; decrement number of pixels remaining in dma buffer
	}
...
.export _RasterSTFPKI_size
	_RasterSTFPKI_size = _RasterSTFPKI_end - _RasterSTFPKI
.export _RasterSTFPKI
.import _FloorDivMod
.import _MPEMDMACmdBuf
.import _MPEPolygonGradient
.import _MPEPolygonScanlineValues
...
;
; Features: clut, bilerp, intensity lighting, chromakey
;
; ru	= texture u-coordinate (LOOP INPUT)
; rv	= texture v-coordinate (LOOP INPUT)
;
...
`ChromaAccept:
	...
	nop												; getting rid of this is difficult
	{
	add_p	v4, v5									; bilerp complete
	addr	v2[3], ru								; increment ru
	dec		rc0										; decrement number of pixels remaining in subspan
	dec		rc1										; decrement number of pixels remaining in dma buffer
	ld_p	(v2[0]), v2								; load next LL color
	...
	{
	bra		c0ne, `RasterLoop2						; loop back for next pixel if pixels remaining in subspan
	mul_p	v4[3], v5, >>#30, v6					; modulate filtered color
	add		v5[3], v4[3]							; increment I
	addr	v3[3], rv								; increment rv
	ld_pz	(v7[0]), v7								; load next UL color with alpha
	}
