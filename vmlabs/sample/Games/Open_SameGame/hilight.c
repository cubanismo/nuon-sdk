...
// This version of high_lite copies blits; it redraws the entire table
// each time
...
// copies in the cursor with centre at (x,y); cursor
// is found in extra.tga
...
{
  if (i < NUM_MATS){
        MyCopyRect(0, 0, MAT_WIDTH-1, MAT_HEIGHT-1, &backgrnd_pic, 
		   UP_LEFT_X, UP_LEFT_Y, &background);
  }
  return 0;
}
...
int read_in_a_mat(int i)
{
  ...
  if (!USE_DATA_FILE){
      ...
      ReadTGAfrmFile(text, backgrnd_pic.dmaFlags, backgrnd_pic.memP);
  }else{
    sprintf(text, "Mat%d.tga", i);
    ReadTGAfrmDatFile(text, backgrnd_pic.dmaFlags, backgrnd_pic.memP);
  }
