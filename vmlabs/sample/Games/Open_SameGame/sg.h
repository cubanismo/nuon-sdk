...
mmlGC gc;
mmlDisplayPixmap screen[3], loading, balls, background, playagain;
mmlDisplayPixmap backgrnd_pic, anim, titlebackgrnd;
mmlDisplayPixmap mats[10];  //keep these for now, though not used
VidDisplay display;
VidChannel mainch, video_ch;

// NOTE: If running this game from a CD-RW or CD-R, one needs 
// ON_DVD set to 1 (to turn off printf statements) and one
// needs USE_DATA_FILE set to 1 as well.
//
// Set to 1 if creating a cof file which will be run from a DVD
// or a CD-RW (when it's set to 1, the printf statements are
// turned off)
#define ON_DVD 1
//
// set to 1 if loading sound and images from sg.dat; 0 otherwise
#define USE_DATA_FILE 1

// NOTE: if writing to CD-R, then set ON_DVD to 1 to get rid of
// printf statement, and correct the DATA_FILE variable below
//
// Have to give different path for DVD version; note that constant
// DATA_FILE only used when USE_DATA_FILE set to 1
#if (ON_DVD)
 #define DATA_FILE "sg.dat"           // was "/udf/nuon/sg.dat"
#else
 #define DATA_FILE "sg.dat"
#endif
...
extern int BOTTOM_RT_X, BOTTOM_RT_Y, TGA_WIDTH, TGA_HEIGHT;
...
int copy_backgrnd_pic_to_backgrnd(int i);
int read_in_a_mat(int i);
...
int Read_CNF_File();
int Read_CNF_FilefrmDatFile(void);
char* find_good_line(char *buf);
int ReadSound(const char *name, struct Sound *sound);
int ReadSoundfrmDatFile(const char *name, struct Sound *sound);
...
// this is where the datafile goes
