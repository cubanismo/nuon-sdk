...
int game(mmlFontContext fc, mmlFont sysP, int n_colours, int special)
{
  ...
  n = 0;
  m = 0;
  // mm keeps track of the current mat number so that the new
  // (randomly chosen) mat is different from the current mat.  A new
  // mat is chosen when the player has completely solved.
  mm = 0;
  old_mm = 0;
  seed = 0;
  ...
  if (MATS == 1){
    ...
    read_in_a_mat(mm);
    copy_backgrnd_pic_to_backgrnd(mm);
  }
  ...
  while (!exit){
    ...
    cur_tBeg = cur_tEnd;     // was cur_tBeg = clock() but this ommited the time
                             // taken to calculate sec above
...
/*********************************************************************/
	...
	if ((num_special_balls() == 0) || 
	  ...
	  if (play_again2(fc, sysP, Score, Best_Scores.Game[count]) == 1){
	    ...
	    // initial number of balls in table
	    num_balls = COL_NUM * ROW_NUM;
	    // count the number of occurances of each colour in the
	    // table (use this later when deciding whether player has
	    // removed significant number of tiles of a certain
	    // colour)
	    for (i = 0; i < n_colours; i++)
	      initial_num_colours[i] = colour_stats(i);
