...
// function to read a sample from a .dat file; it fills in the "sound"
...
int ReadSoundfrmDatFile(const char *name, struct Sound *sound)
{
  ...
  i = find_name_in_Index(name, &Index_Entry);
  if (ON_DVD == 0)
    fprintf(stderr, "Index containing %s is %d\n", name, i);

  if (i < 0){
    // couldn't find sound file; we assume it was left out
    // intentionally and motor along.  (Do print a message though.)
    if (ON_DVD == 0)
      fprintf(stderr, "ReadSoundfrmDatFile: Can't open %s; thus will not be using\n   this clip in the game\n", name);
    sound->data = NULL;
    sound->length = 0;
    return -1;
  }    
  ...
  //  if (get_file("sg.dat", i, (char *)sound->data) < 0){
  if (get_file(DATA_FILE, i, (char *)sound->data) < 0){
    if (ON_DVD == 0)
...
// Play a sound if player removes a large enough group of tiles
int Applaud_Large_Group(int group_num, int initial_n_colours[4], int n_colours)
{
  ...
  if (colour != -1){
    ...
    // determine how big we think "large" should be. Probably needs
    // some tweaking
    q = (n_colours == 3) ? 79 : 70;
