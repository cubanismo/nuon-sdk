...
pline_ptr = RSqrtLUT+768
cs = pline_ptr+4

cinterp = pline_ptr+16   ;won't work with cs+4!  WTFIGO??

iv0 = cinterp+16
iv1 = iv0+16
pline = iv1+16          ;polyline table - 64 longs
trig = pline+256        ;vector for storing trig
max_x = trig+16
genbuf1 = max_x+16       ;128 longs
buildbuf1 = genbuf1

    .origin buildbuf1+512
	buffsize = 32	  
...
randy:	.dc.s	$2574bea7,$a3000000,$83659328,$a3000000
...
;	.include	"dma.s"     ; moved this here to avoid all those warnings
...
;    nop
	...
	mv_s	#64,r0					;length in longs
	jsr	dma_finished		;wait for dma complete
	ld_s	mdmactl,r0	
	add #256,mline              ;update external pointer
    st_s    mline,pline_ptr     ;save external pointer
	mv_s	#pline,mline		;Set mline pointer
    st_s    #64,rc1             ;use counter to trigger loading of longer pline list
...
;    jsr c1eq,loadmore,nop   ;load more polyline data if necessary
    nop
{
    lsr #16,r4,r0
    mv_s    #$8000,r1
...
;    dec rc1
;}
;    jsr c1eq,loadmore,nop   ;load more polyline if needed
;    ld_s    (mline),r4
    bits    #15,>>#0,r4
{
    add #4,mline
...
{
	mv_s	r4,x1
	addm r5,r2,y2			;copy second co-ord pair
	asr	#16,r4,y1				;extract x1
}
{
	mv_s	y2,x2
	lsl	#16,x1				;ready for sign extend/extract y1
}
{
...
{
	asr	#16,y2				;extract x2
}
	st_s	#0,ru
	st_s	#0,rv
	asr	#16,x1
	asr	#16,x2				;complete sign extrension of y co-ords
...

{
	mv_s	r4,y1
	addm r5,r2,y2			;copy second co-ord pair
	asr	#16,r4,x1				;extract x1
}
{
	mv_s	y2,x2
	lsl	#16,y1				;ready for sign extend/extract y1
}
{
	push	v1				;save the polyline info
	lsl	#16,y2
...
{
	asr	#16,x2				;extract x2
}
	st_s	#0,ru
	st_s	#0,rv
	asr	#16,y1
	asr	#16,y2				;complete sign extrension of y co-ords
...
}
{
	...
	sub	temp,temp		;zero this for register move purposes
}
...
{
	subm		y1,y2,delta_y		;get total y delta
	sub	x1,x2,delta_x				;get total x delta
}



	bra	ge,neswap
	add	width,x2,r0					;get max boundary
{
	mv_s	#max_x,r1
...
; new code to support colour interpolation linestyles
    ...
    mv_s    #object+8,r0
{
    ld_p    (r0),v1         ;get colour 1
    add #4,r0
}
{
    ld_p    (r0),v3         ;get colour 2
...
{
	mv_s	delta_y,temp			;save a copy of delta y
...
{
	mv_s	r0,delta_x
	copy	delta_x,dma_xpos		;using dma_xpos as a temp
}
{
...
{
	mv_s	#0,r1
	mul	temp,cosline,>>#0,cosline	;and this is the cosine
	jsr	recip,nop
	abs	r0
}
	sub	#20,r1
...
{
	mv_s	r31,mix
	mul	r0,temp,>>acshift,delta_y		;Y inc per X step 
	copy	width,r0
}
{
	...
	asl	#16,x1,dma_xpos
{
	mv_s	hyp,r0
	jsr	recip					;get 1/h^2
	asl	#16,y1,dma_ypos				;set up initial dma positions
}
...
; set up dma and xy_ctl
	...
	nop
	mv_s	#(dma__cmd+4),r1
	mv_s	#dest_screen+4,r0		;address of dest screen ptr in param block
...

{
	...
	sub	dma_xpos,x2,r24		;set total length of x-span
}
{
	jmp	le,aal_done
	mv_s	#buildbuf1,dma_base	
...
{
 	mv_s	#dmaFlags,r1				;Dest DMA type                                                                                                                                     
	jsr	dma_go					;wait for DMA available
}
	...
	st_s	#0,ru
{
	...
	eor	#1,<>#-8,buffoff			;flip buffer offset
...
;	.segment	local_ram
;	.include	"reciplut.i"		;math tables
;	.include	"rsqrtlut.i"
;	.include	"sinelut.i"
